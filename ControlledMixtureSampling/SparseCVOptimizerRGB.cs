...
/// </summary>
public class SparseCVOptimizerRGB {
    struct CVMatrix {
        public float[] partitionedDiagonal;
        public float[][] partitionedDotOther;
        public float[] otherDotOther;
    }
    CVMatrix matrixR, matrixG, matrixB;
    ...
    /// </param>
    public SparseCVOptimizerRGB(int numPartitioned, int numOther, bool heedPrefix, int maxNumUpdates = 16) {
        void Init(out CVMatrix m) {
            m = new();
            m.partitionedDiagonal = new float[numPartitioned];
            m.partitionedDotOther = new float[numPartitioned][];
            for (int i = 0; i < numPartitioned; ++i) {
                m.partitionedDotOther[i] = new float[numOther];
            }
            m.otherDotOther = new float[(numOther * (numOther + 1)) / 2];
        }
        Init(out matrixR);
        Init(out matrixG);
        Init(out matrixB);
    ...
    public void Clear() {
        void Clear(ref CVMatrix m)
        {
            Array.Clear(m.partitionedDiagonal);
            for (int i = 0; i < numPartitioned; ++i)
                Array.Clear(m.partitionedDotOther[i]);
            Array.Clear(m.otherDotOther);
        }
        Clear(ref matrixR);
        Clear(ref matrixG);
        Clear(ref matrixB);
                       ...
                       float samplingPdf, RgbColor throughputWeight)
    {
        ...
        RgbColor prefixWeight = RgbColor.White;
        if (heedPrefix)
        {
            balanceEstimate *= throughputWeight * throughputWeight;
            prefixWeight = throughputWeight * throughputWeight;
        }

        Debug.Assert(float.IsFinite(prefixWeight.Average));
        ...
        int idx = 0;
        RgbColor r = prefixWeight * invSumPdf / samplingPdf;
        for (int i = 0; i < numOther; ++i)
        {
            for (int j = i; j < numOther; ++j)
            {
                Atomic.AddFloat(ref matrixR.otherDotOther[idx], otherPdfs[j] * otherPdfs[i] * r.R);
                Atomic.AddFloat(ref matrixG.otherDotOther[idx], otherPdfs[j] * otherPdfs[i] * r.G);
                Atomic.AddFloat(ref matrixB.otherDotOther[idx], otherPdfs[j] * otherPdfs[i] * r.B);
                idx++;
            }
        }
        ...
        Atomic.AddFloat(ref matrixR.partitionedDiagonal[partIdx], partPdf * r.R);
        Atomic.AddFloat(ref matrixG.partitionedDiagonal[partIdx], partPdf * r.G);
        Atomic.AddFloat(ref matrixB.partitionedDiagonal[partIdx], partPdf * r.B);
        for (int i = 0; i < numOther; ++i)
        {
            Atomic.AddFloat(ref matrixR.partitionedDotOther[partIdx][i], otherPdfs[i] * r.R);
            Atomic.AddFloat(ref matrixG.partitionedDotOther[partIdx][i], otherPdfs[i] * r.G);
            Atomic.AddFloat(ref matrixB.partitionedDotOther[partIdx][i], otherPdfs[i] * r.B);
        }
#else
        lock (this)
        {
    ...
    public void Solve() {
        ...
        float[] SolveColor(ref CVMatrix m, float[] contribs)
        {
            ...
            for (int i = 0; i < numOther; ++i)
            {
                ...
                for (int j = 0; j < numOther; ++j)
                {
                    ...
                    for (int k = 0; k < numPartitioned; ++k)
                        sumRatios += m.partitionedDotOther[k][i] * m.partitionedDotOther[k][j] / m.partitionedDiagonal[k];
                    ...
                    float other = m.otherDotOther[row * numOther - (row * (row + 1)) / 2 + col];
            ...
            for (int i = 0; i < numOther; ++i)
            {
                ...
                for (int k = 0; k < numPartitioned; ++k)
                {
                    if (m.partitionedDiagonal[k] == 0)
                        continue;
                    sumWeights += m.partitionedDotOther[k][i] * contribs[k] / m.partitionedDiagonal[k];
                }
            ...
            for (int i = 0; i < numPartitioned; ++i)
            {
                ...
                else
                {
                    ...
                    {
                        sum += m.partitionedDotOther[i][k] * Y[k];
                    }
                    X[i] = (contribs[i] - sum) / m.partitionedDiagonal[i];
                }
            }
        ...
        coeffsR = SolveColor(ref matrixR, contributionsRed);
        coeffsG = SolveColor(ref matrixG, contributionsGreen);
        coeffsB = SolveColor(ref matrixB, contributionsBlue);
#endif
